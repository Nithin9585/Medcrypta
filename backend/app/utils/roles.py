...
class User(ABC):
    ...
    def verify(self) -> bool:
        ...
        try:
            ...
            if not user_doc:
                logger.warning(
                    f"User '{self.username}' not found in collection '{self.__class__.credentials_collection}'."
                )
                return False
            ...
            try:
                ...
                logger.error(
                    f"Error decrypting password for user '{self.username}': {e}"
                )
                return False
            ...
            if stored_password == self.password:
                ...
                logger.warning(
                    f"Password mismatch for user '{self.username}'."
                )
                return False

        except Exception as e:
            logger.error(
                f"Error during verification for user '{self.username}': {e}"
            )
            return False
    ...
    def register(cls, username: str, password: str, details: dict) -> bool:
        ...
        try:
            logger.info(
                f"Inserting credentials for user '{username}' into '{cls.credentials_collection}' collection."
            )
        ...
        try:
            logger.info(
                f"Inserting details for user '{username}' into '{cls.details_collection}' collection."
            )
        ...
        logger.info(
            f"User '{username}' registered successfully in credentials '{cls.credentials_collection}' and details '{cls.details_collection}'."
        )
    ...
    def get_details(cls, username: str) -> dict:
        ...
        try:
            ...
            if not result:
                logger.warning(
                    f"No details found for user '{username}' in collection '{cls.details_collection}'."
                )
                return None
            return cls.convert_objectid(result)
        except Exception as e:
            logger.error(
                f"Error retrieving details for user '{username}': {e}"
            )
            return None
    ...
    def verify(self) -> bool:
        ...
        try:
            ...
            if doctor_doc is None:
                logger.warning(
                    f"No details found for doctor '{self.username}' in collection '{self.__class__.details_collection}'."
                )
                return False
    ...
    def verify(self) -> bool:
        ...
        try:
            ...
            if pharmacy_doc is None:
                logger.warning(
                    f"No details found for pharmacy '{self.username}' in collection '{self.__class__.details_collection}'."
                )
                return False
    ...
    def verify(self) -> bool:
        ...
        try:
            ...
            if validator_doc is None:
                logger.warning(
                    f"No details found for validator '{self.username}' in collection '{self.__class__.details_collection}'."
                )
                return False
